...
knitr::include_graphics(here::here("_manuscript", "Figures", "preprocessing_fig1.png"))
...
gaze_sub_id <- gaze_sub_id %>%
  ...
  group_by(subject, trial, time_bin, condition, condition1) %>%
  summarise(Looks = as.integer(any(Looks == 1)), .groups = "drop")
```
...
ggsave(here::here("_manuscript", "Figures", "Figure6.png"), width=12, height=18, dpi=500)
...
cpa.lme <-  permutes::clusterperm.glmer(Looks~ condition1_code + (1|subject), data=gaze_sub_L2_cp1, series.var=~time_bin, nperm = total_perms)
```
