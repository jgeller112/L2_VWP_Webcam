...
Despite the differences between research-grade and consumer grade eye-tracking, a number of studies have begun to look at if lab-based results replicate online using webcam eye-tracking. Most relevant to this tutorial are online replications using the VWP [@tanenhaus1995; cf. @cooper1974]. For the past 25 years, the VWP has been a dominant force in language research, helping researchers tackle a wide range of topics, including sentence processing [@eberhard1995], word recognition [@allopenna1998], bilingualism [@ito2018], and the effects of brain damage on language [@mirman2012].
...
The `webgazeR` package is installed in `Code Block 1` from Github using the `remotes` package.
...
In addition, we indicate other variables for aggregation, such as condition, and use the newly created timebins variable, which represents the time intervals over which we aggregate data. The resulting downsampled dataset, output as @tbl-agg-sub, provides a simplified and more concise view of gaze patterns, making it easier to analyze and interpret broader trends.
...
gaze_sub_L2 %>%
  head() %>%
  tt() %>%
  save_tt(here::here("_manuscript","Figures", "downsample_table.png"), overwrite=TRUE)

knitr::include_graphics(here::here("_manuscript", "Figures", "downsample_table.png"))
```
...
gaze_sub_L2_id <- downsample_gaze(gaze_sub_L2_long, bin.length=100, timevar="time", aggvars=c("subject", "condition", "condition1", "time_bin"))
```
...
Below we have plotted the time course data in @fig-L2comp for each condition. By default the graphs are using a color-blind friendly palette from the `ggokabeito` package [@ggokabeito]. Because these are `ggplot` objects, you can further modify if you choose.

```{r}
#| label: fig-L2comp
#| fig-cap: "Comparison of L2 competition effect in the Spanish-Spanish condition, the Spanish-English condition, and the no competition condition."
#| fig-width: 12
#| echo: false
...
#| echo: false
knitr::include_graphics(here::here("_manuscript", "Figures", "gorilla_cords.png"))
...
To preform CPT, we will load in the `permutaes` [@permutes], `permuco` [@permuco], and `foreach` [@foreach] packages in R so we can use the `cluster.glmer`() function to run a cluster permutation model with our `gaze_sub_id` dataset where each row in `Looks` denotes whether the AOI was fixated, with values of zero (not fixated) or one (fixated).
...
@fig-clustermass shows a significant cluster (shaded) for both the Spanish-Spanish and Spanish-English conditions. We see there is one significant cluster in both conditions.
...
ggsave(here::here("_manuscript", "Figures", "clust_combined_plot.png"), dpi=300, width = 12, height=8)
```

# Discussion

Webcam eye-tracking is a relatively nascent technology, and as such, there is limited guidance available for researchers. To ameliorate this, we created a tutorial to assist new users of visual world webcam eye-tracking, using some of the best practices laid out in other work (e.g.,[@bramlett2024]). To further facilitate this process, we created the `webgazeR` package, which contains several helper functions designed to streamline data preprocessing, analysis, and visualization.
...
Our conceptual replication findings are highly encouraging, demonstrating competition effects both within (Spanish-Spanish condition) and across languages (Spanish-English condition), closely paralleling the results reported by @sarrett2022. However, several important methodological and sample differences warrant discussion.

A key methodological difference between our study and @sarrett2022 lies in the approach used to analyze the time course of competition. While @sarrett2022 employed a non-linear curve-fitting method [@mcmurray2010], we used a CPA. This methodological distinction limits our ability to address similar temporal questions. Nonetheless, the overall temporal patterns are strikingly similar. For instance, our CPA revealed a significant cluster starting at 500 ms, whereas @sarrett2022 identified competition effects emerging at approximately 400 ms. This indicates a delay of about 100 ms in competition onset between lab-based and online eye-tracking data. This delay, while notable, reflects a significant improvement over previous webcam-based studies [e.g., @slim2023; @slim2024]. It is important to emphasize, however, that CPA clusters cannot reliably be used for temporal inferences without [@fields2019].

Our study also employed a truncated stimulus set, with only 250 trials compared to the 450 trials in @sarrett2022. Despite this reduction, the number of trials in our study remains larger than most existing webcam-based studies. Even with the smaller set, we observed a similar pattern of competition effects in both the Spanish-Spanish and Spanish-English conditions, demonstrating the robustness of our findings.

Another notable difference is the recruitment strategy and participant screening. @sarrett2022 recruited participants from a Spanish college course and used the LexTALE-Spanish assessment [@izura2014] to evaluate Spanish proficiency. In contrast, our data were collected via Prolific with limited filters, which only allowed us to screen for native language and experience with another language. This constraint limited our ability to refine participant selection further and likely contributed to differences in participant profiles. While @sarrett2022 focused on adult L2 learners with known language proficiency levels, our sample included a broader range of L2 speakers with limited checks on their language abilities. This may help explain why we did not observe a cohort competition effect that persisted across the time course as reported by @sarrett2022.
...
It is important to note here that Gorilla uses WebGazer.js [@papoutsaki2016] to perform it’s eye tracking. It is unclear if poor calibration results from the noise introduced by participants' environments/equipments or if it is a function of the method itself, or both. We have listed some equipment and environmental factors that may contribute to the poor performance; however it could be the algorithm itself that is poor. There are other experimental platforms out there that use different eye-tracking ML algorithms to perform webcam eye-tracking (e.g., labvanced; [@kaduk2024]). In labvanced, comapred to Gorilla they use head motion tracking that measures the distance of the participant in front the screen to ensure head movement is restricted to an acceptable range. Together this might make for a better eye-tracking experience with less data thrown out. This should be investigated further. 
...
final_table %>%
  tt() %>%
  save_tt(here::here("_manuscript", "Figures",  "poor_good_table.png"), overwrite = TRUE)
