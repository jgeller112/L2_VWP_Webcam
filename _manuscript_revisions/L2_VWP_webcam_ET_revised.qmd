...
**One online method that is growing in popularity—and that may help mitigate some of the aforementioned limitations by increasing the inclusivity and representativeness of participant samples—is automated webcam-based eye-tracking. Webcam eye-tracking typically requires only three things: (1) a personal computer, (2) an internet connection, and (3) a built-in or external webcam. Gaze data is collected directly through a web browser without requiring any additional software installation, making it highly accessible.**
...
**The purpose of this tutorial is to walk users through the key preprocessing steps required to analyze WebGazer.js data, using the widely adopted, open-source statistical environment R [@R]. The `webgazeR` package complements the lead author’s existing `gazeR` package [@geller2020], which is commonly used to analyze gaze and pupil data from lab-based eye-tracking studies. Here, we extend this work to the online context by providing tools specifically tailored to the challenges and structure of webcam data.**
